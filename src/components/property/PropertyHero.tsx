...
export default function PropertyHero({ property }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 px-6 md:px-12 lg:px-20 xl:px-28 pb-16 md:pb-24 w-full">
        <motion.div
        ...
        >
          ...
          <h1 className="font-display text-4xl md:text-6xl lg:text-7xl text-primary-foreground font-medium leading-tight mb-2">{property.title}</h1>
          <p className="font-body text-sm md:text-base tracking-[0.15em] uppercase text-primary-foreground/70 mb-6">{property.city}</p>
          <p className="font-display text-3xl md:text-4xl text-primary-foreground font-light mb-8">{formatPrice(property.price)}</p>
          <div className="flex items-center gap-8 mb-10">
            {property.beds && (
              <div className="text-primary-foreground">
                <span className="font-display text-3xl md:text-4xl">{property.beds}</span>
                <span className="font-body text-xs tracking-[0.15em] uppercase ml-2 text-primary-foreground/70">Beds</span>
              </div>
            )}
            {property.baths && (
              <div className="text-primary-foreground">
                <span className="font-display text-3xl md:text-4xl">{property.baths}</span>
                <span className="font-body text-xs tracking-[0.15em] uppercase ml-2 text-primary-foreground/70">Baths</span>
              </div>
            )}
            {property.sqft && (
              <div className="text-primary-foreground">
                <span className="font-display text-3xl md:text-4xl">{property.sqft.toLocaleString()}</span>
